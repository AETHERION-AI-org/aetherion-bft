...
func initValidatorSet(polyBFTConfig PolyBFTConfig, transition *state.Transition) error {
	...
	// Use validator address as rootchain manager in standalone mode (no bridge)
	rootChainManager := types.ZeroAddress
	if polyBFTConfig.Bridge != nil {
		rootChainManager = polyBFTConfig.Bridge.CustomSupernetManagerAddr
	} else if len(polyBFTConfig.InitialValidatorSet) > 0 {
		rootChainManager = polyBFTConfig.InitialValidatorSet[0].Address
	}
	...
	initFn := &contractsapi.InitializeValidatorSetFn{
		...
		NewRootChainManager: rootChainManager,
...
func initRewardPool(polybftConfig PolyBFTConfig, transition *state.Transition) error {
	initFn := &contractsapi.InitializeRewardPoolFn{
		...
		NewBaseReward:   func() *big.Int { r, _ := new(big.Int).SetString(polybftConfig.EpochReward, 10); if r == nil { r = big.NewInt(1) }; return r }(),
	}
