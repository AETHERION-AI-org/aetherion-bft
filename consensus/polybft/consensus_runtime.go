...
func (c *consensusRuntime) initStakeManager(logger hcf.Logger, dbTx *bolt.Tx) error {
	// Standalone mode: no bridge means no rootchain stake manager.
	if !c.IsBridgeEnabled() {
		if contracts.IsAetherionStakingForkConfigured() {
			// Aetherion native staking fork: rebuild the validator set from the on-chain
			// registry each epoch. The manager is epoch-gated internally — before the fork
			// epoch its UpdateValidatorSet returns an empty delta, so it behaves exactly
			// like the dummy (frozen) set and a resyncing node reproduces history.
			c.stakeManager = newNativeStakeManager(
				logger.Named("native-stake-manager"),
				c.config.blockchain,
				contracts.AetherionValidatorRegistryContract,
				int(c.config.PolyBFTConfig.MaxValidatorSetSize),
			)
		} else {
			c.stakeManager = &dummyStakeManager{}
		}
