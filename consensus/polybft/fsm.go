...
var (
	...
	errDistributeEmissionTxDoesNotExist = errors.New("distribute emission transaction is " +
		"not found in the epoch ending block, but the Aetherion emission fork is active")
	errDistributeEmissionTxNotExpected = errors.New("didn't expect distribute emission transaction " +
		"in a non epoch ending block")
	errDistributeEmissionTxSingleExpected = errors.New("only one distribute emission transaction is " +
		"allowed in an epoch ending block")
	errDistributeEmissionTxForkNotActive = errors.New("found distribute emission transaction " +
		"before the Aetherion emission fork is active")
	errValidatorRewardsTxDoesNotExist = errors.New("distribute validator rewards transaction is " +
		"not found in the epoch ending block, but the Aetherion staking fork is active")
	errValidatorRewardsTxNotExpected = errors.New("didn't expect distribute validator rewards " +
		"transaction in a non epoch ending block")
	errValidatorRewardsTxSingleExpected = errors.New("only one distribute validator rewards " +
		"transaction is allowed in an epoch ending block")
	errValidatorRewardsTxForkNotActive = errors.New("found distribute validator rewards transaction " +
...
func (f *fsm) BuildProposal(currentRound uint64) ([]byte, error) {
	...
	if f.isEndOfEpoch {
		...
		if err := f.applyDistributeEmissionTx(); err != nil {
			return nil, err
		}
...
// applyDistributeEmissionTx builds and writes the per-epoch AETH emission distribution
// state transaction, once the Aetherion emission fork is active for this epoch
// (contracts.IsAetherionEmissionForkActive). Before activation this is a no-op, so an
// unconfigured/pre-fork binary behaves exactly as it does today.
func (f *fsm) applyDistributeEmissionTx() error {
	if !contracts.IsAetherionEmissionForkActive(f.epochNumber) {
		return nil
	}

	tx, err := f.createDistributeEmissionTx()
	if err != nil {
		return fmt.Errorf("failed to create distribute emission transaction: %w", err)
	}

	if err := f.blockBuilder.WriteTx(tx); err != nil {
		return fmt.Errorf("failed to apply distribute emission transaction: %w", err)
	}

	return nil
}

// createDistributeEmissionTx creates a StateTransaction, which invokes the
// AetherionEmissionDistributor contract's distributeEpoch(epochId). The contract itself
// never reverts for the system caller under any condition (empty treasury, repeat
// epoch, pause, misconfiguration are all no-op skips with an event) — see
// contracts/contracts/AetherionEmissionDistributor.sol — and even if it somehow did,
// an EVM-level revert only marks this one receipt failed, it does not invalidate the
// block (state.Transition.Write sets ReceiptFailed and returns nil error; verified
// against the existing RewardPool distribution path, which relies on the same
// property). The one liveness-sensitive step performed by this Go function is ABI
// encoding a single uint256, which cannot fail for any valid epoch number.
func (f *fsm) createDistributeEmissionTx() (*types.Transaction, error) {
	input, err := (&contractsapi.DistributeEmissionFn{
	...
	return createStateTransactionWithData(f.Height(), contracts.AetherionEmissionDistributorContract, input), nil
}

// applyDistributeValidatorRewardsTx builds and writes the per-epoch validator reward
// state transaction, once the Aetherion staking fork is active for this epoch and the
// rewards contract is configured (contracts.IsAetherionValidatorRewardsActive). Before
// that it is a no-op, so a pre-fork/unconfigured binary behaves exactly as it does today.
func (f *fsm) applyDistributeValidatorRewardsTx() error {
	if !contracts.IsAetherionValidatorRewardsActive(f.epochNumber) {
		return nil
	}

	tx, err := f.createDistributeValidatorRewardsTx()
	if err != nil {
		return fmt.Errorf("failed to create distribute validator rewards transaction: %w", err)
	}

	if err := f.blockBuilder.WriteTx(tx); err != nil {
		return fmt.Errorf("failed to apply distribute validator rewards transaction: %w", err)
	}

	return nil
}

// aetherionValidatorRewardAmount is the node (validator) slice of this epoch's emission
// that the validator reward transaction distributes: the raw halving-scheduled epoch
// reward times the node basis points. Pure function of the epoch, so every validator
// computes the same amount and the transaction hashes match.
func aetherionValidatorRewardAmount(epochID uint64) *big.Int {
	amount := new(big.Int).Mul(
		state.RawAetherionEpochReward(epochID),
		new(big.Int).SetUint64(contracts.AetherionNodeRewardBps),
	)

	return amount.Div(amount, big.NewInt(10_000))
}

// createDistributeValidatorRewardsTx creates a StateTransaction which invokes
// AetherionValidatorRewards.distribute(epochId, amount). The contract never reverts for
// the system caller under any condition (idempotent per epoch; empty NodeVault, no active
// validators and pause are all no-op skips with an event) — see
// contracts/contracts/AetherionValidatorRewards.sol — and even if it somehow did, an
// EVM-level revert only marks this one receipt failed, it does not invalidate the block
// (the same liveness property the emission and legacy reward paths rely on). Both the
// epoch and the amount are pure functions of the epoch number, so every validator
// reproduces the identical transaction.
func (f *fsm) createDistributeValidatorRewardsTx() (*types.Transaction, error) {
	input, err := (&contractsapi.DistributeValidatorRewardsFn{
		EpochID: new(big.Int).SetUint64(f.epochNumber),
		Amount:  aetherionValidatorRewardAmount(f.epochNumber),
	}).EncodeAbi()
	if err != nil {
		return nil, err
	}

	return createStateTransactionWithData(f.Height(), contracts.AetherionValidatorRewardsContract, input), nil
}

// verifyDistributeValidatorRewardsTx recreates the validator reward transaction locally
// and compares hashes, so a proposer cannot slip in a different epoch or a different
// amount than the schedule dictates.
func (f *fsm) verifyDistributeValidatorRewardsTx(validatorRewardsTx *types.Transaction) error {
	if !f.isEndOfEpoch {
		return errValidatorRewardsTxNotExpected
	}

	if !contracts.IsAetherionValidatorRewardsActive(f.epochNumber) {
		return errValidatorRewardsTxForkNotActive
	}

	localTx, err := f.createDistributeValidatorRewardsTx()
	if err != nil {
		return err
	}

	if validatorRewardsTx.Hash != localTx.Hash {
		return errValidatorRewardsTxAmountMismatch
	}
...
func (f *fsm) VerifyStateTransactions(transactions []*types.Transaction) error {
	...
	for _, tx := range transactions {
		...
		case *contractsapi.DistributeEmissionFn:
			if distributeEmissionTxExists {
				// if we already validated distribute emission tx,
				// that means someone added more than one distribute emission tx to block,
				// which is invalid
				return errDistributeEmissionTxSingleExpected
			}

			distributeEmissionTxExists = true

			if err := f.verifyDistributeEmissionTx(tx); err != nil {
				return fmt.Errorf("error while verifying distribute emission transaction. error: %w", err)
			}
		case *contractsapi.DistributeValidatorRewardsFn:
			if validatorRewardsTxExists {
				// only one distribute validator rewards tx is allowed per epoch ending block
				return errValidatorRewardsTxSingleExpected
			}

			validatorRewardsTxExists = true
	...
	if f.isEndOfEpoch {
		...
		if contracts.IsAetherionEmissionForkActive(f.epochNumber) && !distributeEmissionTxExists {
			// mandatory once the fork is active, exactly like commit epoch / distribute
			// rewards above; before activation this check does not apply at all
			return errDistributeEmissionTxDoesNotExist
		}

		if contracts.IsAetherionValidatorRewardsActive(f.epochNumber) && !validatorRewardsTxExists {
...
// verifyDistributeEmissionTx creates the distribute emission transaction locally and
// compares its hash with the one extracted from the block. Rejects the tx outright
// outside epoch-ending blocks, and before the Aetherion emission fork is active for
// this epoch — a malicious proposer must not be able to sneak emission payouts into
// arbitrary blocks or ahead of the scheduled activation epoch.
func (f *fsm) verifyDistributeEmissionTx(distributeEmissionTx *types.Transaction) error {
	if !f.isEndOfEpoch {
		return errDistributeEmissionTxNotExpected
	}

	if !contracts.IsAetherionEmissionForkActive(f.epochNumber) {
		return errDistributeEmissionTxForkNotActive
	}

	localDistributeEmissionTx, err := f.createDistributeEmissionTx()
	if err != nil {
		return err
	}

	if distributeEmissionTx.Hash != localDistributeEmissionTx.Hash {
		return fmt.Errorf(
			"invalid distribute emission transaction. Expected '%s', but got '%s' distribute emission hash",
			localDistributeEmissionTx.Hash,
			distributeEmissionTx.Hash,
		)
	}
